...
class QbPassYdsMlPredictions(Base):
    __table__ = Table(
        'qb_pass_yds_ml_predictions',
        metadata,
        ...
        Column('previous_season_pass_yds', Integer),
        Column('ensemble_predicted_pass_yds', Integer),
        Column('gradient_boosting_predicted_pass_yds', Integer),
        Column('lasso_predicted_pass_yds', Integer),
        Column('lightgbm_predicted_pass_yds', Integer),
        Column('linear_regression_predicted_pass_yds', Integer),
        Column('neural_network_predicted_pass_yds', Integer),
        Column('random_forest_predicted_pass_yds', Integer),
        Column('ridge_predicted_pass_yds', Integer),
    ...
    __table_args__ = (
        UniqueConstraint('master_player_id', 'prediction_season', name='qb_pass_yds_pred_uk'),
        {'schema': 'machine_learning'}
    )

    def __repr__(self):
        return f"<QbPassYdsMlPredictions(id={self.id}, master_player_id={self.master_player_id})>"
    ...
    def upsert_prediction(cls, db_session, prediction_data: Dict) -> None:
        ...
        try:
            ...
            # Add ON CONFLICT clause for upsert behavior using the composite unique constraint
            stmt = stmt.on_conflict_do_update(
                constraint='qb_pass_yds_pred_uk',
    ...
    def bulk_upsert_predictions(cls, db_session, predictions_data: List[Dict]) -> None:
        ...
        """
        try:
            # Ensure date fields are set for all records
            ...
            # Create the bulk upsert statement
            ...
            stmt = stmt.on_conflict_do_update(
                constraint='qb_pass_yds_pred_uk',
                set_={
                    'previous_season_pass_yds': stmt.excluded.previous_season_pass_yds,
                    'ensemble_predicted_pass_yds': stmt.excluded.ensemble_predicted_pass_yds,
                    'gradient_boosting_predicted_pass_yds': stmt.excluded.gradient_boosting_predicted_pass_yds,
                    'lasso_predicted_pass_yds': stmt.excluded.lasso_predicted_pass_yds,
                    'lightgbm_predicted_pass_yds': stmt.excluded.lightgbm_predicted_pass_yds,
                    'linear_regression_predicted_pass_yds': stmt.excluded.linear_regression_predicted_pass_yds,
                    'neural_network_predicted_pass_yds': stmt.excluded.neural_network_predicted_pass_yds,
                    'random_forest_predicted_pass_yds': stmt.excluded.random_forest_predicted_pass_yds,
                    'ridge_predicted_pass_yds': stmt.excluded.ridge_predicted_pass_yds,
