...
class RbRushYdsMlPredictions(Base):
    __table__ = Table(
        'rb_rush_yds_ml_predictions',
        metadata,
    ...
    __table_args__ = (
        UniqueConstraint('master_player_id', 'prediction_season', name='rb_rush_yds_pred_uk'),
        {'schema': 'machine_learning'}
    )

    def __repr__(self):
        return f"<RbRushYdsMlPredictions(id={self.id}, master_player_id={self.master_player_id})>"
    ...
    def upsert_prediction(cls, db_session, prediction_data: Dict) -> None:
        ...
        try:
            ...
            stmt = stmt.on_conflict_do_update(
                constraint='rb_rush_yds_pred_uk',
